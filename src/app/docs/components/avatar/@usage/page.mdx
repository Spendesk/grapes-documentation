import { Avatar } from "@dev-spendesk/grapes";
import { Preview } from "@/lib/preview/preview";

## Variant

The Avatar comes with two visual styles: circle and square. You should use a `circle` avatar to represent a person, and a `square` avatar to represent an entity, such as a company.

<Preview>
  <Avatar variant="circle" src="/aurelien.webp" text="Michael Murphy" />
  <Avatar variant="square" src="/aurelien.webp" text="Michael Murphy" />
</Preview>

```tsx sandbox
<>
  <Avatar variant="circle" src="/aurelien.webp" text="Michael Murphy" />
  <Avatar variant="square" src="/aurelien.webp" text="Michael Murphy" />
</>
```

## Size

The Avatar comes in five different sizes: xs, s, m (default), l, and xl.

<Preview>
  <Avatar size="xs" src="/aurelien.webp" text="Michael Murphy" />
  <Avatar size="s" src="/aurelien.webp" text="Michael Murphy" />
  <Avatar size="m" src="/aurelien.webp" text="Michael Murphy" />
  <Avatar size="l" src="/aurelien.webp" text="Michael Murphy" />
  <Avatar size="xl" src="/aurelien.webp" text="Michael Murphy" />
</Preview>

```tsx sandbox
<>
  <Avatar size="xs" src="/aurelien.webp" text="Michael Murphy" />
  <Avatar size="s" src="/aurelien.webp" text="Michael Murphy" />
  <Avatar size="m" src="/aurelien.webp" text="Michael Murphy" />
  <Avatar size="l" src="/aurelien.webp" text="Michael Murphy" />
  <Avatar size="xl" src="/aurelien.webp" text="Michael Murphy" />
</>
```

## Badge

A badge can be used with an avatar to display additional information, such as a supplier in an expense claim situation.

<Preview>
  <Avatar
    badgeProps={{
      src: "/chloe.webp",
      text: "Laura Lagarde",
      variant: "square",
    }}
    size="m"
    text="Michael Murphy"
    variant="circle"
  />
  <Avatar
    badgeProps={{
      src: "/chloe.webp",
      text: "Laura Lagarde",
      variant: "circle",
    }}
    size="m"
    text="Michael Murphy"
    variant="square"
  />
</Preview>

```tsx sandbox
<>
  <Avatar
    badgeProps={{
      src: "/chloe.webp",
      text: "Laura Lagarde",
      variant: "square",
    }}
    size="m"
    text="Michael Murphy"
    variant="circle"
  />
  <Avatar
    badgeProps={{
      src: "/chloe.webp",
      text: "Laura Lagarde",
      variant: "circle",
    }}
    size="m"
    text="Michael Murphy"
    variant="square"
  />
</>
```

## Fallback

If there is an error loading the src of the avatar or if the `src` has not been specified, there are 2 fallbacks:

- If there's a `fallbackSrc` prop, we use it.
- If there's no `fallbackSrc` prop, we use the text props to generate the initials and a random color for both the background and the text color.

<Preview>
  <Avatar
    fallbackSrc="/supplier.svg"
    size="m"
    text="Spendesk"
    variant="square"
  />
  <Avatar size="m" text="George Gray" />
  <Avatar size="m" text="Lewis Barker" />
  <Avatar size="m" text="Michael Murphy" />
  <Avatar size="m" text="Nayden Lennart" />
</Preview>

```tsx sandbox
<>
  <Avatar
    fallbackSrc="/supplier.svg"
    size="m"
    text="Spendesk"
    variant="square"
  />
  <Avatar size="m" text="George Gray" />
  <Avatar size="m" text="Lewis Barker" />
  <Avatar size="m" text="Michael Murphy" />
  <Avatar size="m" text="Nayden Lennart" />
</>
```
