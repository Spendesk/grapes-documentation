...
It means that `Popover`, `Select` and `DropdownMenu` are not compatible with the CollapsibleList component, as they can't overflow the list limits.
It is recommended to avoid using these components within a CollapsibleList to ensure proper functionality.
:::
